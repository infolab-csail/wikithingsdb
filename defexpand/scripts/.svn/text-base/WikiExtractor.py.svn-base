...
def WikiDocument(out, id, title, infoboxes, text):
    url = get_url(id, prefix)
    header = '<doc id="%s" url="%s" title="%s" infobox="%s">\n' % (id, url, title, ', '.join(infoboxes))
...
def process_data(input, output):
    ...
    inText = False
    redirect = False
    infobox_summary = {'pages_with_infobox': [], 'infoboxes': {}}
    counter = 0
    not_ignored_counter = 0
    for line in input:
    ...
    print 'Total number of pages: ' + str(counter)
    if summary_dir is None:
        print 'Number of pages not filtered out:' + str(not_ignored_counter)
        print infobox_summary_to_string(infobox_summary)
    else:
        json.dump(infobox_summary, summary_dir)
        summary_dir.close()


def ignore_page(data):
    ...
    is_refer = referto_pattern in data['abstract']
...
def update_infobox_summary(infobox_summary, infoboxes, id, title):
    title = title.encode('utf-8')
    if len(infoboxes) > 0:
        infobox_summary['pages_with_infobox'].append((id, title))
    for infobox in infoboxes:
        if (id, title) not in infobox_summary['infoboxes'].get(infobox, []):
            infobox_summary['infoboxes'][infobox] = infobox_summary['infoboxes'].get(infobox, []) + [(id, title)]
            #print 'Add to infobox "' + infobox + '" artitle "' + title + '"'


def infobox_summary_to_string(infobox_summary):
    string = 'Number of pages with infoboxes: %d\n' % (len(infobox_summary['pages_with_infobox']))
    string += 'Number of infoboxes: %d\n' % (len(infobox_summary['infoboxes']))
    string += 'Infoboxes:\n'
    for (infobox, pages) in sorted(infobox_summary['infoboxes'].items(), key=lambda x: -len(x[1])):
        titles = [title for (id, title) in pages]
        string += '"%s" (%d) --> %s\n' % (infobox, len(titles), titles)
    return string
...
def main():
    global keepLinks, keepSections, prefix, acceptedNamespaces, limit, summary_dir
    script_name = os.path.basename(sys.argv[0])

    try:
        long_opts = ['help', 'compress', 'bytes=', 'basename=', 'links', 'ns=', 'sections', 'output=', 'version', 'summary=', 'numpages=', 'filtered']
        opts, args = getopt.gnu_getopt(sys.argv[1:], 'cb:hln:o:B:svS:N:f', long_opts)
    ...
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            ...
            try:
                ...
                summary_dir = open(arg, 'w')
        elif opt in ('-N', '--numpages'):
                limit = int(arg)
        elif opt in ('-f', '--filtered'):
                filtered = True
